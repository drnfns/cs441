...
=== definition
let $f$ and $g$ be functions from the set of integers or the set of real numbers to the set of real numbers. we say that $f(x)$ is $O(g(x))$ if there are constants $C$ and $k$ such that $|f(x)| <= C|g(x)|$ whenever $x > k$. we read this as: "$f(x)$ is big-o of $g(x)$".
...
=== definition
let $f$ and $g$ be functions from the set of integers or the set of real numbers to the set of real numbers. we say that $f(x)$ is $Omega(g(x))$ if there are constants $C$ and $k$ with $C$ _positive_ such that $f(x) >= C|g(x)|$ whenever $x > k$. we read this as: "$f(x)$ is big-omega of $g(x)$".

*aside*: there is a connection between big-o and big-omega. specifically, $f(x)$ is $Omega(g(x))$ _if and only if_ $g(x)$ is $O(f(x))$.

== big-theta notation

=== definition
let $f$ and $g$ be functions from the set of integers or the set of real numbers to the set of real numbers. we say that $f(x)$ is $Theta(g(x))$ if $f(x)$ is $O(g(x))$ and $f(x)$ is $Omega(g(x))$. when $f(x)$ is $Theta(g(x))$, we say that $f$ is big-theta of $g(x)$, that $f(x)$ is of _order_ $g(x)$, and that $f(x)$ and $g(x)$ are of the _same order_.