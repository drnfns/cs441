...
#show: conf.with(id: "alj213", name: "Alex Jin", title: "rec_3", date: datetime(
  year: 2025,
  month: 9,
  day: 18,
))
...
+ Predicates:
  ...
  Proposition:
  $forall x forall y thin ((U(x) and A(y)) -> exists z thin (M(z, y) -> R(x)))$
+ Predicates:
  - $C(c) equiv$ "$c$ is a console"
  - $F(f) equiv$ "$f$ is a fault condition"
  - $A(f, c) equiv$ "console $c$ is accessible during fault $f$"
  Proposition: $forall x thin (F(x) -> exists y thin (C(y)and A(x, y)))$
+ Predicates:
  - $P(p) equiv$ "$p$ is an endpoint"
  - $R(r) equiv$ "$r$ is a path"
  - $C(r, a, b) equiv$ "path $r$ connects point $a$ and point $b$"
  Proposition:
  $forall p_1 forall p_2 thin ((P(p_1) and P(p_2) and not (p_1 equiv p_2)) -> exists r_1 exists r_2 thin (R(r_1) and R(r_2) and not (r_1 equiv r_2) and C(r_1, p_1, p_2) and C(r_2, p_1, p_2)))$
...
+ $forall x thin (not (x equiv "Lina") -> B("Lina", x))$
+ Predicate: $P(x) equiv$ "$x$ is presenting on the board." \
  Propoposition: $forall x thin (not (x equiv P(x)) -> N(x, P(x)))$
...
+ There exists a CS441 student that attends recitation more than all the CS441
  TAs.
+ There exists a CS441 TA that enjoys CS441 more than all the other TAs.
...
== Answer
+ Given: $r$, $r -> not f$, and $not f -> s$, with the conclusion $s$. Using
  hypothetical syllogism, we can get that $r -> s$. So, by modus ponens, if it
  rained today, then I am sad.
+ Given: $not p$, $(c and not t) -> (e and p)$, and $c or n$, with the
  conclusion $t or n$. First use contraposition to get that
  $(not e or not p) -> (not c or t)$. Given $not p$, we can get $(not c or t)$.
  Given that $c or n$, we can get $t or n$ using resolution.
+ Given: $exists x thin (C(x) and G(x))$, $forall x thin (L(x) or R(x))$,
  $forall x thin (L(x) -> not G(x)))$, with the conslusion
  $exists x thin (C(x) and R(x))$. We first use existential instantiation to
  conclude that $C(a) and G(a)$ for an element $a$, and then since $G(a)$ is
  true, we can conclude that $not L(a)$ is true by contraposition. Finally,
  since $not L(a)$ is true, the second statement is also true since is an
  disjunction, which means that they also like ice cream.
...
+ Hypothesis: $forall x in RR thin ((n > 1) -> (n^2 > 1))$ \
  Conclusion: $forall x in RR thin ((n^2 > 1) -> (n > 1))$ \
  Incorrect, $(p -> q) arrow.r.not (q -> p)$ (the converse is not logically
  equivalent).
+ Hypothesis: $forall n in RR thin ((n > 3) -> (n^2 > 9))$ \
  Conclusion: $forall n in RR thin ((n^2 <= 9) -> (n <= 3))$ \
  Correct, modus tollens. The conclusion is also just a contrapositive of the
  original statement.
+ Hypothesis: $forall n in RR ((n > 2) -> (n^2 > 4))$ \
  Conclusion: $forall n in RR ((n <=2) -> (n^2 <= 4))$ \
  Incorrect, the inverse of the proposition is not logically equivalent to the
  original statement.
